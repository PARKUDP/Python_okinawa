...
def filter_data(result_table, filter_entry):
    filter_input = filter_entry.get().lower()
    for row in result_table.get_children():
        if filter_input in result_table.item(row, 'values')[0].lower():
            result_table.item(row, tags=('visible',))
        else:
            result_table.item(row, tags=('hidden',))
    result_table.tag_configure('visible', foreground='black')
    result_table.tag_configure('hidden', foreground='white')
...
def search_data(search_entry, root):
    ...
    filter_button = ttk.Button(filter_frame, text="フィルタ適用", command=lambda: filter_data(result_table, filter_entry))
